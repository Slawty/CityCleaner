...
public class DirtlingWander : MonoBehaviour
{
    ...
    [FormerlySerializedAs("reanchorWanderOnMovementResume")]
    ...
    void Update()
    {
        if (stateController.CurrentState != DirtlingState.Wandering)
            return;

        if (!stateController.WanderingEnabled)
            return;
